...
resource "aws_route53_zone" "viacash_fr" {
  name = "viacash.fr"
}
...
resource "aws_route53_record" "viacash_fr_caa_combined" {
  zone_id = aws_route53_zone.viacash_fr.zone_id
  name    = ""
  type    = "CAA"
  ttl     = 3600
  records = [
...
# Disallows sending mails on behalf of viacash.fr
resource "aws_route53_record" "viacash_fr_spf" {
  zone_id = aws_route53_zone.viacash_fr.zone_id
  name    = ""
  type    = "TXT"
  ttl     = 3600
...
# DMARC TXT record
resource "aws_route53_record" "viacash_fr_dmarc" {
  zone_id = aws_route53_zone.viacash_fr.zone_id
  name    = "_dmarc"
  type    = "TXT"
  ttl     = 3600

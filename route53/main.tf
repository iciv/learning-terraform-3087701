...
resource "aws_route53_zone" "viacash_es" {
  name = "viacash.es"
}
...
resource "aws_route53_record" "viacash_es_caa_combined" {
  zone_id = aws_route53_zone.viacash_es.zone_id
  name    = ""
  type    = "CAA"
  ttl     = 3600
  records = [
...
# Disallows sending mails on behalf of viacash.es
resource "aws_route53_record" "viacash_es_spf" {
  zone_id = aws_route53_zone.viacash_es.zone_id
  name    = ""
  type    = "TXT"
  ttl     = 3600
...
# DMARC TXT record
resource "aws_route53_record" "viacash_es_dmarc" {
  zone_id = aws_route53_zone.viacash_es.zone_id
  name    = "_dmarc"
  type    = "TXT"
  ttl     = 3600

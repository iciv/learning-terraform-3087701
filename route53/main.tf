...
resource "aws_route53_zone" "viacash_gr" {
  name = "viacash.gr"
}
...
resource "aws_route53_record" "viacash_gr_caa_combined" {
  zone_id = aws_route53_zone.viacash_gr.zone_id
  name    = ""
  type    = "CAA"
  ttl     = 3600
  records = [
...
# Disallows sending mails on behalf of viacash.gr
resource "aws_route53_record" "viacash_gr_spf" {
  zone_id = aws_route53_zone.viacash_gr.zone_id
  name    = ""
  type    = "TXT"
  ttl     = 3600
...
# DMARC TXT record
resource "aws_route53_record" "viacash_gr_dmarc" {
  zone_id = aws_route53_zone.viacash_gr.zone_id
  name    = "_dmarc"
  type    = "TXT"
  ttl     = 3600

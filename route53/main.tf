...
resource "aws_route53_zone" "viacash_pt" {
  name = "viacash.pt"
}
...
resource "aws_route53_record" "viacash_pt_caa_combined" {
  zone_id = aws_route53_zone.viacash_pt.zone_id
  name    = ""
  type    = "CAA"
  ttl     = 3600
  records = [
...
# Disallows sending mails on behalf of viacash.pt
resource "aws_route53_record" "viacash_pt_spf" {
  zone_id = aws_route53_zone.viacash_pt.zone_id
  name    = ""
  type    = "TXT"
  ttl     = 3600
...
# DMARC TXT record
resource "aws_route53_record" "viacash_pt_dmarc" {
  zone_id = aws_route53_zone.viacash_pt.zone_id
  name    = "_dmarc"
  type    = "TXT"
  ttl     = 3600

...
resource "aws_route53_zone" "viacash_it" {
  name = "viacash.it"
}
...
resource "aws_route53_record" "viacash_it_caa_combined" {
  zone_id = aws_route53_zone.viacash_it.zone_id
  name    = ""
  type    = "CAA"
  ttl     = 3600
  records = [
...
# Disallows sending mails on behalf of viacash.it
resource "aws_route53_record" "viacash_it_spf" {
  zone_id = aws_route53_zone.viacash_it.zone_id
  name    = ""
  type    = "TXT"
  ttl     = 3600
...
# DMARC TXT record
resource "aws_route53_record" "viacash_it_dmarc" {
  zone_id = aws_route53_zone.viacash_it.zone_id
  name    = "_dmarc"
  type    = "TXT"
  ttl     = 3600

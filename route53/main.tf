...
resource "aws_route53_zone" "viacash_uk" {
  name = "viacash.uk"
}
...
resource "aws_route53_record" "viacash_uk_caa_combined" {
  zone_id = aws_route53_zone.viacash_uk.zone_id
  name    = ""
  type    = "CAA"
  ttl     = 3600
  records = [
...
# Disallows sending mails on behalf of viacash.uk
resource "aws_route53_record" "viacash_uk_spf" {
  zone_id = aws_route53_zone.viacash_uk.zone_id
  name    = ""
  type    = "TXT"
  ttl     = 3600
...
# DMARC TXT record
resource "aws_route53_record" "viacash_uk_dmarc" {
  zone_id = aws_route53_zone.viacash_uk.zone_id
  name    = "_dmarc"
  type    = "TXT"
  ttl     = 3600
